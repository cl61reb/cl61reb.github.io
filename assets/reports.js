...
window.REPORTS = [
  {
    ...
    href: "2025.html",
    title: "2025",
    blurb: "May to December 2025, taken from the agreed schedule spreadsheet. Fixed history — this report is never regenerated.",
    dataUrl: "../data/2025-data.json",
    // No exceptionsUrl / usualScheduleUrl: there is no calendar behind this
    // period to find gaps in, and no rotation baseline to compare against.
    // Marks the report as never refreshed. scripts/refresh-all.mjs leaves its
    // data alone; only re-running the extraction script changes it.
    static: true,
    // The menu card's flag line. Without one it would claim every day has a
    // calendar entry, which is not what this data is.
    menuFlag: "From the agreed schedule spreadsheet",
    rangeNote:
      "Taken from the agreed schedule spreadsheet (schedule.01062026.xlsx, 'Data' tab), covering 1 May to 31 December 2025. This period predates the joint calendar's custody entries, so it is fixed history rather than a live calculation and is not regenerated by the weekly refresh.",
  },
  {
    kind: "custody",
    id: "ytd",
